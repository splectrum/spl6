  ...
  createClient (opts) {
    ...
    process.nextTick(() => {
      ...
      if (opts && opts.server) {
        ...
        let pts = [new streamx.PassThrough(), new streamx.PassThrough()];
...
class Client extends events.EventEmitter {
  ...
  createChannel (transport, opts) {
    ...
    if (typeof transport == 'function') {
      ...
      if (!objectMode) {
        // streamx has no unpipe. Suppress pipeline cleanup errors on the
        // transport streams — they are expected when a connection closes.
        decoder.on('error', () => {});
        encoder.on('error', () => {});
      }
    }
...
class FrameDecoder extends streamx.Transform {
  constructor () {
    super();
    this._id = undefined;
...
class FrameEncoder extends streamx.Transform {
  constructor () {
    super();
    this.on('finish', function () { this.push(null); });
  }
...
class NettyDecoder extends streamx.Transform {
  constructor () {
    super();
    this._id = undefined;
    this._frameCount = 0;
...
class NettyEncoder extends streamx.Transform {
  constructor () {
    super();
    this.on('finish', function () { this.push(null); });
  }
